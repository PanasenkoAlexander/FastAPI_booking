...
class HotelDAO(BaseDAO):
    ...
    async def find_all(cls, location: str, date_from: date, date_to: date):
        ...
        get_hotels_with_rooms = (
            # Код ниже можно было бы прописать так:
            # select(
            #     Hotels,
            #     booked_hotels.c.rooms_left,
            # )
            # но т.к. используется конструкция Hotels.__table__.columns, то алхимия отдает все столбцы по одному
            # если передать всю модель Hotels и один дополнительный столбец rooms_left, то будет проблематично Pydantic
            # распарсить такую структуру данных, т.е. проблема кроется именно в парсинге ответа алхимии Пайдентиком.
            select(
